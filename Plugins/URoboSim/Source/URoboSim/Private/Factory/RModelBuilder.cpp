...
void URModelBuilder::Load(USDFModel* InModelDescription, ARModel* OutModel,FVector InLocation)
{
  ...
  if(OutModel)
    {
      UE_LOG(LogTemp, Log, TEXT("Loading robot model..."));
      Model = OutModel;
  ...
  for(auto& Joint : Model->Joints)
    {
      ...
      UE_LOG(LogTemp, Log, TEXT("Setting Parent-Child relationship between %s - %s"), *Joint.Value->ParentName, *Joint.Value->ChildName);
      SetConstraintPosition(Joint.Value);
      Joint.Value->Constraint->ConnectToComponents();

      if(!Child->bAttachedToParent)
        {
          UE_LOG(LogTemp, Log, TEXT("Attaching %s to %s"), *Joint.Value->ParentName, *Joint.Value->ChildName);
